...
const pieColors = ['#5683d2', '#00d638', '#e4f222', '#999ba3', '#ff492c', '#9b59b6'];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px' }}>
        <MetricCard icon={<DollarSign size={18} />} label="Total cobrado mes" value={`$${totalMes.toLocaleString('es-AR')}`} color="var(--accent-green)" />
        <MetricCard icon={<TrendingUp size={18} />} label="Pendiente de cobro" value={`$${pendiente.toLocaleString('es-AR')}`} color="var(--accent-orange)" />
        <MetricCard icon={<Users size={18} />} label="Consultas" value={String(totalConsultas)} color="var(--accent-blue)" />
        <MetricCard icon={<BarChart2 size={18} />} label="Ticket promedio" value={`$${ticketProm.toLocaleString('es-AR')}`} color="var(--accent-yellow)" />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '3fr 2fr', gap: '16px' }}>
        <div style={{
          ...
          <ResponsiveContainer width="100%" height={160}>
            <LineChart data={lineData} margin={{ left: -20, right: 8, top: 4, bottom: 0 }}>
              ...
              />
              <Line type="monotone" dataKey="monto" stroke="var(--accent-yellow)" strokeWidth={2} dot={false} isAnimationActive={false} />
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        <div style={{
          ...
          <h3 style={{ margin: '0 0 8px', fontSize: '13px', fontWeight: 500, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
            POR OBRA SOCIAL
          </h3>
          <ResponsiveContainer width="100%" height={160}>
            <PieChart>
      ...
      <div style={{
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <h3 style={{ margin: 0, fontSize: '13px', fontWeight: 500, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em', flex: 1 }}>
            MOVIMIENTOS
          </h3>
          {(['todos', 'cobrado', 'pendiente', 'a-facturar'] as FiltroEstado[]).map(f => (
            <button
              ...
              style={{
                ...
                background: filtro === f ? 'var(--accent-yellow)' : 'transparent',
                color: filtro === f ? 'var(--bg-canvas)' : 'var(--text-secondary)',
              }}
            >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '100px 1fr 140px 100px 100px 80px', padding: '8px 20px', borderBottom: '1px solid var(--border-subtle)' }}>
      ...
      <div style={{
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border-subtle)' }}>
          <h3 style={{ margin: 0, fontSize: '13px', fontWeight: 500, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
            LIQUIDACIONES OBRAS SOCIALES — JUNIO 2026
          </h3>
        </div>
        {liquidaciones.map(l => (
          <div
          ...
          >
            ...
            <span style={{
              fontSize: '11px', borderRadius: '999px', padding: '3px 10px',
              background: l.estado === 'Cobrada' ? 'rgba(0,214,56,0.15)' : l.estado === 'Presentada' ? 'rgba(86,131,210,0.15)' : 'rgba(255,73,44,0.15)',
              color: l.estado === 'Cobrada' ? 'var(--accent-green)' : l.estado === 'Presentada' ? 'var(--accent-blue)' : 'var(--accent-orange)',
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px', color }}>
        {icon}
        <span style={{ fontSize: '11px', fontWeight: 500, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}</span>
      </div>
      <div style={{ fontSize: '24px', fontWeight: 500, color: 'var(--text-primary)', fontVariantNumeric: 'tabular-nums' }}>{value}</div>
    </div>
  );
}
